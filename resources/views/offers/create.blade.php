    ...
    <div x-data="offerSystem()" class="space-y-6">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-8">
            <div class="flex items-center gap-4">
                <div
                    class="w-16 h-16 bg-primary rounded-2xl flex items-center justify-center text-white shadow-lg shadow-primary/20 rotate-3">
                    <span class="material-symbols-outlined text-[32px]">edit_document</span>
                </div>
                <div>
                    <h2 class="text-4xl font-black text-text-primary tracking-tighter leading-none">Offer Generator</h2>
                    <p class="text-[10px] uppercase tracking-[0.2em] text-primary font-black mt-1 opacity-70">Premium
                        Letter Pad Workflow</p>
                </div>
            </div>
            <div class="flex items-center gap-4">
                ...
                <div
                    ...
                    <span class="text-[9px] uppercase tracking-widest font-black text-text-secondary opacity-50">Offer
                        Date</span>
                    <input type="date" name="offer_date" form="offerForm" value="{{ date('Y-m-d') }}"
                                            ...
                                            <div class="grid grid-cols-2 gap-4 mb-6 text-[12px]">
                                                ...
                                                <div class="text-right">
                                                    <p><strong>Date:</strong> <span
                                                            class="text-emerald-900 font-bold">{{ date('d/m/Y') }}</span>
                                                    </p>
                                                </div>
                                            </div>
                                        </template>
    ...
    <footer
        ...
        <div class="flex items-center gap-4 w-full md:w-auto">
            <button type="submit" form="offerForm"
                class="primary-btn px-12 h-14 rounded-2xl shadow-xl shadow-primary/20 hover:shadow-2xl hover:shadow-primary/40 group w-full md:w-auto">
                <span class="material-symbols-outlined group-hover:rotate-12 transition-transform">verified</span>
                Generate & Save Offer
            </button>
        </div>
    </footer>
    ...
    <script>
        function offerSystem() {
            return {
                ...
                customers: [],
                referenceNo: {!! json_encode(old('reference_no', '')) !!},
                subject: {!! json_encode(old('subject', 'Plant Supply Offer')) !!},
                greeting: {!! json_encode(old('greeting', 'Dear Sir/Madam,')) !!},
                introText: {!! json_encode(old('intro_text', 'As per your requirement, we are pleased to offer the following plants for your consideration.')) !!},
                ...
                init() {
                    ...
                    const oldItems = oldItemsRaw ? Object.values(oldItemsRaw) : [];
                    this.items = oldItems.length ? oldItems.map(i => ({
