...
<style>
    ...
    .document-wrapper {
        font-family: 'NotoSansGujarati', 'Helvetica', 'Arial', sans-serif;
        color: #111827;
        background: #fff;
        margin: 0;
        padding: 0;
    ...
    .document-wrapper.a5 { font-size: 10px; }
    .document-wrapper.letter { font-size: 11px; }
    ...
    .brand-cell { text-align: center; vertical-align: middle; }
    .contact-cell { width: 140px; text-align: right; vertical-align: middle; font-size: 13px; font-weight: bold; color: #166534; }
    ...
    .subtitle { font-size: 10px; color: #4b5563; font-weight: 600; margin: 1px 0 4px 0; }
    .address-badge {
        font-size: 9px;
        background: #166534;
        color: white;
    ...
    /* Meta Table */
    .meta-table { width: 100%; margin-bottom: 20px; border-collapse: collapse; table-layout: auto; }
    .bill-to-box { background: #f9fafb; padding: 10px; border-radius: 8px; border: 1px solid #d1d5db; vertical-align: top; width: 55%; }
    .inv-details-box { vertical-align: top; padding-left: 20px; width: 45%; }

    .detail-row { border-bottom: 1px dashed #d1d5db; padding: 6px 0; width: 100%; }
    .detail-label { font-weight: 700; color: #4b5563; font-size: 11px; text-transform: uppercase; }
    .detail-value { font-weight: 900; color: #111827; float: right; }

    /* Items Table */
    .items-table { width: 100%; border-collapse: collapse; margin-bottom: 15px; table-layout: auto; }
    .items-table th {
        background: #f3f4f6;
        padding: 8px 12px;
    ...
    .col-sr { text-align: center; }
    .col-qty { text-align: center; font-weight: bold; }
    .col-rate { text-align: right; }
    .col-amt { text-align: right; font-weight: bold; }

    .item-name { font-weight: bold; color: #111827; font-size: 13px; word-wrap: break-word; }
    .item-meta { font-size: 11px; color: #4b5563; font-style: italic; margin-top: 2px; }

    /* Totals */
    .totals-area { width: 100%; border-collapse: collapse; margin-top: 15px; }
    .notes-cell { width: 60%; vertical-align: top; font-size: 11px; font-style: italic; color: #4b5563; }
    .totals-cell { width: 40%; vertical-align: top; }

    .summary-row { padding: 6px 0; border-bottom: 1px dashed #e5e7eb; width: 100%; }
    .summary-label { font-weight: 700; color: #4b5563; }
    .summary-val { font-weight: 800; color: #111827; float: right; }
    ...
    .grand-total-val { font-size: 18px; font-weight: 900; float: right; }
                    ...
                    <table class="meta-table">
                        <tr>
                            <td class="bill-to-box">
                                <div style="font-weight: 800; color: #4b5563; font-size: 10px; margin-bottom: 5px; text-transform: uppercase;">Bill To:</div>
                                <div style="font-size: 18px; font-weight: 900; color: #111827;">{{ $invoice->customer_name }}</div>
                                @if($invoice->phone)
                                    <div style="font-size: 13px; color: #4b5563; font-weight: 700; margin-top: 2px;">Mobile: +91 {{ $invoice->phone }}</div>
                                @endif
                            </td>
                    ...
                    <tbody>
                        @if($p_idx > 0)
                            <tr style="background: #f8fafc; font-style: italic; font-weight: bold; color: #166534;">
                                <td colspan="4" style="text-align: right; padding-right: 15px;">Brought Forward (B/F) -></td>
                                <td class="col-amt"><span class="currency-sym">₹</span> {{ number_format($running_total, 2) }}</td>
                            </tr>
                        @endif
                        ...
                        @foreach($p_items as $idx => $item)
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @php $details = []; @endphp
                                    @if($item->height && $item->height !== '-') @php $details[] = 'H: '.$item->height; @endphp @endif
                                    @if($item->bag_size && $item->bag_size !== '-') @php $details[] = 'Bag: '.$item->bag_size; @endphp @endif
                                    @if(count($details) > 0)
                                        <span class="item-meta"> - {{ implode(' | ', $details) }}</span>
                                    @endif
                                </td>
                                <td class="col-qty">{{ $item->quantity }}</td>
                                <td class="col-rate"><span class="currency-sym">₹</span> {{ number_format($item->price, 2) }}</td>
                                <td class="col-amt"><span class="currency-sym">₹</span> {{ number_format($item_total, 2) }}</td>
                            </tr>
                        @endforeach
                        ...
                        @if(!$is_last)
                            <tr style="background: #f8fafc; font-style: italic; font-weight: bold; color: #166534;">
                                <td colspan="4" style="text-align: right; padding-right: 15px;">Carried Forward (C/F) -></td>
                                <td class="col-amt"><span class="currency-sym">₹</span> {{ number_format($running_total, 2) }}</td>
                            </tr>
                        @endif
                    </tbody>
                        ...
                        <tr>
                            <td class="notes-cell">
                                @if($invoice->notes)
                                    <div style="margin-top: 10px; padding-right: 20px;">
                                        <strong style="color: #374151; text-transform: uppercase; font-size: 10px;">Note:</strong><br>
                                        {{ $invoice->notes }}
                                    </div>
                                @endif
                            </td>
                            <td class="totals-cell">
                                <div class="summary-row clearfix">
                                    ...
                                    <span class="summary-val"><span class="currency-sym">₹</span> {{ number_format($invoice->subtotal, 2) }}</span>
                                </div>
                                @if($invoice->discount > 0)
                                    <div class="summary-row clearfix" style="color: #dc2626; background: #fef2f2; padding: 4px 6px; margin: 2px -6px; border-radius: 4px;">
                                        <span class="summary-label" style="color: #dc2626;">Discount</span>
                                        <span class="summary-val">- <span class="currency-sym">₹</span> {{ number_format($invoice->discount, 2) }}</span>
                                    </div>
                                @endif
                                @if($invoice->gst_amount > 0)
                                    <div class="summary-row clearfix">
                                        <span class="summary-label">CGST</span>
                                        <span class="summary-val"><span class="currency-sym">₹</span> {{ number_format($invoice->cgst, 2) }}</span>
                                    </div>
                                    <div class="summary-row clearfix">
                                        <span class="summary-label">SGST</span>
                                        <span class="summary-val"><span class="currency-sym">₹</span> {{ number_format($invoice->sgst, 2) }}</span>
                                    </div>
                                @endif
                                <div class="grand-total-row clearfix">
                                    <span class="grand-total-label">Grand Total</span>
                                    <span class="grand-total-val"><span class="currency-sym">₹</span> {{ number_format($invoice->total, 2) }}</span>
                                </div>
                    ...
                    <table class="footer-table">
                        <tr>
                            ...
                            <td class="signature-cell">
                                ...
                                <div style="font-size: 11px; color: #6b7280; margin: 30px 0 6px 0; font-weight: 700;">(ઓથોરાઈઝ્ડ સહી)</div>
