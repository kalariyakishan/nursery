<aside
    class="h-screen w-64 fixed left-0 top-0 bg-white border-r border-border-light flex flex-col z-50 overflow-y-auto transition-transform duration-300 transform md:translate-x-0"
    ...
    <div class="h-20 flex items-center justify-between px-6 border-b border-border-light/50">
        <div class="flex items-center gap-3">
            <div
                class="w-12 h-12 rounded-lg bg-white border border-border-light shadow-sm flex items-center justify-center p-1.5 shrink-0 overflow-hidden group hover:rotate-3 transition-transform duration-300">
                <img src="{{ asset('images/logo.png') }}" class="w-full h-full object-contain"
                    alt="New Vrundavan Nursery Logo">
            </div>
            <div class="min-w-0">
                <h1 class="text-sm font-bold text-text-primary gujarati-text leading-tight truncate">New Vrundavan
                    Nursery</h1>
                <p class="text-[9px] font-semibold text-text-secondary uppercase tracking-[0.15em] opacity-40">Digital
                    Billing System</p>
            </div>
        </div>
        <!-- Close button on Mobile -->
        <button @click="sidebarOpen = false"
            class="md:hidden text-text-secondary p-1 border border-border-light rounded-lg">
    ...
    <nav class="flex-1 px-4 py-2 space-y-1">
        ...
        <div x-data="{ invoiceOpen: {{ request()->routeIs('invoices.*') ? 'true' : 'false' }} }">
            <button @click="invoiceOpen = !invoiceOpen"
                ...
                <div class="flex items-center gap-3">
                    <span class="material-symbols-outlined text-[20px]"
                        style="{{ request()->routeIs('invoices.*') ? "font-variation-settings: 'FILL' 1;" : '' }}">receipt_long</span>
                ...
                <span class="material-symbols-outlined text-[18px] transition-transform duration-200"
                    :class="invoiceOpen ? 'rotate-180' : ''">expand_more</span>
            </button>

            <div x-show="invoiceOpen" x-transition:enter="transition ease-out duration-200"
        ...
        <div x-data="{ offerOpen: {{ request()->routeIs('offers.*') ? 'true' : 'false' }} }">
            <button @click="offerOpen = !offerOpen"
                ...
                <div class="flex items-center gap-3">
                    <span class="material-symbols-outlined text-[20px]"
                        style="{{ request()->routeIs('offers.*') ? "font-variation-settings: 'FILL' 1;" : '' }}">description</span>
                    <span class="gujarati-text text-sm">ઓફર / એસ્ટિમેટ</span>
                </div>
                <span class="material-symbols-outlined text-[18px] transition-transform duration-200"
                    :class="offerOpen ? 'rotate-180' : ''">expand_more</span>
            </button>

            <div x-show="offerOpen" x-transition:enter="transition ease-out duration-200"
                x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0"
                class="ml-10 mt-1 border-l border-border-light/60 pl-2">
        ...
        <div x-data="{ rojmelOpen: {{ request()->routeIs('rojmel.*') ? 'true' : 'false' }} }">
            <button @click="rojmelOpen = !rojmelOpen"
                ...
                <div class="flex items-center gap-3">
                    <span class="material-symbols-outlined text-[20px]"
                        style="{{ request()->routeIs('rojmel.*') ? "font-variation-settings: 'FILL' 1;" : '' }}">account_balance_wallet</span>
                    <span class="gujarati-text text-sm">ડેઈલી રોકડમેળ</span>
                </div>
                <span class="material-symbols-outlined text-[18px] transition-transform duration-200"
                    :class="rojmelOpen ? 'rotate-180' : ''">expand_more</span>
            </button>

            <div x-show="rojmelOpen" x-transition:enter="transition ease-out duration-200"
                x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0"
                class="ml-10 mt-1 border-l border-border-light/60 pl-2">
        ...
        <div
            x-data="{ labourOpen: {{ request()->routeIs('labour-entries.*', 'advances.*', 'workers.*', 'reports.labour*') ? 'true' : 'false' }} }">
            <button @click="labourOpen = !labourOpen"
                ...
                <div class="flex items-center gap-3">
                    <span class="material-symbols-outlined text-[20px]"
                        style="{{ request()->routeIs('labour-entries.*', 'advances.*', 'workers.*', 'reports.labour*') ? "font-variation-settings: 'FILL' 1;" : '' }}">engineering</span>
                    <span class="gujarati-text text-sm">મજૂરી મેનેજમેન્ટ</span>
                </div>
                <span class="material-symbols-outlined text-[18px] transition-transform duration-200"
                    :class="labourOpen ? 'rotate-180' : ''">expand_more</span>
            </button>

            <div x-show="labourOpen" x-transition:enter="transition ease-out duration-200"
                x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0"
                class="ml-10 mt-1 border-l border-border-light/60 pl-2">
        ...
        <a href="{{ route('settings.index') }}"
            ...
            <span class="material-symbols-outlined text-[20px]"
                style="{{ request()->routeIs('settings.index') ? "font-variation-settings: 'FILL' 1;" : '' }}">settings</span>
    ...
    <div class="p-4 border-t border-border-light">
        <div class="flex items-center gap-3 px-4 py-3 mb-2 rounded-lg bg-background/50">
            <div
                class="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-xs shrink-0">
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit"
                class="w-full flex items-center gap-3 px-4 py-2.5 text-red-600 hover:bg-red-50 transition-colors rounded-lg text-sm font-semibold group">
                <span
                    class="material-symbols-outlined text-[20px] group-hover:rotate-12 transition-transform">logout</span>
